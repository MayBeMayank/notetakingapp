# Delta Spec — public-share-view
**Change:** AB-1008
**FRS coverage:** FRS-7.3, 7.4, 7.6 (view side), 7.8; expiry/revoke enforcement on view (7.2/7.5); cross-cutting §9.4
**SDS coverage:** §6.2, §8, §5.1

> The owner endpoints — generate (`POST /api/notes/:id/shares`, FRS-7.1/7.2), list active links (`GET /api/shares`, FRS-7.7), and revoke (`DELETE /api/shares/:id`, FRS-7.5) — live in the sibling `share-links` delta of this change and are **not** specified here. This delta covers only the **public, unauthenticated read** at `GET /api/public/notes/:token`.
