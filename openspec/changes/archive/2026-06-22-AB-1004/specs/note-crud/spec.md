...
> Deferred to other tickets and therefore **not** specified here: version snapshot-on-save (FRS-4.1.4 / 4.3.2 → AB-1009), tag associations on create/update (FRS-4.3.1 tags clause → AB-1006), full list sort/filter/status (FRS-4.5.2–4.5.4 → AB-1005), and background purge of expired-window notes (FRS-4.4.4 → ops / scheduled cron job, not a feature ticket). FRS-4.4.2 has three halves — AB-1004 covers the default-**list** exclusion of soft-deleted notes; the **search-results** exclusion lands with AB-1007 and the **tag-count** exclusion with AB-1006 (those endpoints do not exist in AB-1004).
...
> AB-1004 scope is the default list only. Configurable `sort` / `order`, multi-tag AND filtering, and `status=trashed` are added in AB-1005.
...
> The "tag associations" clause of FRS-4.3.1 and the version snapshot of FRS-4.3.2 are out of AB-1004 scope (AB-1006 / AB-1009).
