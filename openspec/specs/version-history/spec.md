...
Define a note's version history: snapshot-on-save, listing, viewing, and non-destructive restore of versions, with most-recent-50 retention and owner-only privacy.
...
**ADRs:** ADR-003 (NoteVersion tag-id snapshot / restore re-applies surviving tags), ADR-004 (version reads allowed on a soft-deleted note; restore blocked)

> The **snapshot-on-save** behaviour that produces these versions is specified in `note-crud` (create writes v1; update writes a version on title/content change). The version-detail `tagIds` snapshot relies on the `NoteVersion.tagIds` column in `prisma-schema`. Public share-link exclusion of versions (FRS-8.6) is enforced on the public endpoint owned by AB-1008; this capability adds no public route.
