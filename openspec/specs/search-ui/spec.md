...
reproduce the same results. Each debounced query change and page change pushes
a new history entry (no `replace`).
...
`<mark>` elements rendered visibly highlighted. Snippets are rendered via a
safe parser (`renderSnippet`) that splits on `<mark>`/`</mark>` tokens and
produces React `<mark>` elements — no `dangerouslySetInnerHTML`. (FRS-6.4)

#### Scenario: result with matched terms
- **WHEN** a search result contains a snippet like `"…the <mark>meeting</mark> agenda…"`
- **THEN** the word "meeting" is rendered visually highlighted (accent background via `mark` CSS)

#### Scenario: result with title only (no content match)
- **WHEN** a search result has an empty or whitespace snippet
- **THEN** the snippet area is hidden; the title is still displayed
...
- **THEN** an instructional empty state is displayed and no API call is made
...
- **THEN** a skeleton is displayed in the results area (`role="status"`)
